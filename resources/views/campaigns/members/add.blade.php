...
@section('content')
    ...
    <div class="container">
        {{ Breadcrumbs::render('campaigns.view.addmembers', $campaign) }}
        @if(session('message'))
            <p class="alert alert-info">{!! session('message') !!}</p>
        @endif
        <h4>Add Members (Influencers) to {{($campaign->name)}}</h4>
        <div class="card  mb-3" >

            <div class="card-header">

                <form class="d-flex float-right" action="{{ route('campaigns.addmembers', $campaign->id) }}" method="GET">
            ...
            <div class="card-body">
                <form action="{{ route('campaigns.addmembers', $campaign->id) }}" method="POST">
                    @csrf
                    <table class="table">
                    <thead>
                    <tr>
